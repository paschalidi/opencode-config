...
## Your cover letter is stored at

Read `~/.config/opencode/instructions/cover-letter.md` to get the base content.
...
Use the voice characteristics defined in the system prompt. The user's voice is:
...
2. Feed it back to the cover letter instructions
3. Ensure the tone matches the rest of the letter

The cover letter instructions at `~/.config/opencode/instructions/cover-letter.md` should reference this skill for "experience paragraphs" when the user needs to add specific, authentic work stories.

## Cover letter workflow (still supported)

If the user wants a cover letter:
1. Ask for the job description
2. Read the base cover letter from `~/.config/opencode/instructions/cover-letter.md`
3. Tailor it — swap company, role, tech stack, reorder highlights
4. Keep the voice, structure, ~80% of wording intact
5. Never fabricate experience or metrics
6. Output to chat, offer to save as `cover-letter-{company}-tailored.md`
