...
2. **Create the parent** with `jira_create_issue`:
   - `issue_type`: `Task` (unless the user requested otherwise — `Story`, `Bug`, `Epic`, etc.).
   - `description`: Markdown. Headings as `## Heading`, lists as `-`, inline code with backticks. Jira will translate to wiki markup automatically.
...
3. **Create each subtask** with `jira_create_issue`:
...
- **Never assume the project key.** Ask if not stated.
